...
pub async fn incremental_sync(
    ...
    tracing::info!("[Command] incremental_sync called, identity_id={}", identity_id);
    ...
    match result {
        Ok(r) => {
            let _ = app.emit("sync-progress", SyncProgressFrontend::success(r.total_new_count));
            tracing::info!("[Command] incremental_sync completed, new_items={}", r.total_new_count);
            Ok(SyncProgressFrontend::success(r.total_new_count))
        }
        Err(e) => {
            let err_str = e.to_string();
            if let Some((image, execution)) = parse_captcha_marker(&err_str, "MANUAL_CAPTCHA_REQUIRED|") {
...
pub async fn full_sync(
    ...
    let sync_service = state.sync_service.read().await;
    let result = sync_service.full_sync_identity(identity_id, None).await
        .map_err(|e| {
            let err_str = e.to_string();
    ...
    let _ = app.emit("sync-progress", SyncProgressFrontend::success(result.total_new_count));
    tracing::info!("[Command] full_sync completed, new_items={}", result.total_new_count);
...
pub async fn sync_with_captcha(
    ...
    tracing::info!("[Command] sync_with_captcha called, identity_id={}, captcha_len={}", identity_id, captcha_code.len());
    ...
    match sync_service.sync_with_captcha(identity_id, &captcha_code, &execution).await {
        Ok(result) => {
            let _ = app.emit("sync-progress", SyncProgressFrontend::success(result.total_new_count));
            tracing::info!("[Command] sync_with_captcha SUCCESS, new_items={}", result.total_new_count);
            Ok(SyncProgressFrontend::success(result.total_new_count))
        }
        Err(e) => {
            let err_str = e.to_string();
...
pub async fn refresh_captcha(
    ...
    match sync_service.get_captcha_for_manual_login().await {
        Ok((image, execution)) => {
            ...
            Ok(SyncProgressFrontend::captcha_required(image, execution, "请输入验证码"))
        }
        Err(e) => {
...
pub async fn cas_login(
    ...
    let account = db.get_account_by_student_id(&account_id)
        .map_err(|e| { tracing::error!("[Command] cas_login: account not found: {}", e); e.to_string() })?
        .ok_or_else(|| { tracing::error!("[Command] cas_login: account {} does not exist", account_id); "账号不存在".to_string() })?;

    let sync_service = state.sync_service.read().await;
    sync_service.login_with_captcha(&account, &captcha_code).await
        .map_err(|e| { tracing::error!("[Command] cas_login FAILED for {}: {}", account_id, e); e.to_string() })?;
...
pub async fn check_login_status(
    ...
    tracing::debug!("[Command] check_login_status called, account_id={}", account_id);
    
    let db = state.db_manager.read().await;
    let crypto = state.crypto.read().await;
    let session = db.get_session(&account_id, &crypto)
        .map_err(|e| { tracing::error!("[Command] check_login_status: get_session failed: {}", e); e.to_string() })?;
